...
const SERVICE_STYLE: Record<string, string> = {
  http:     "bg-sev-info/15 text-sev-info",
  https:    "bg-sev-low/15 text-sev-low",
  ssh:      "bg-muted text-muted-foreground",
  smb:      "bg-sev-critical/15 text-sev-critical",
  ftp:      "bg-sev-high/15 text-sev-high",
  smtp:     "bg-sev-medium/15 text-sev-medium",
  pop3:     "bg-sev-medium/15 text-sev-medium",
  imap:     "bg-sev-medium/15 text-sev-medium",
  mysql:    "bg-sev-info/15 text-sev-info",
  postgres: "bg-sev-info/15 text-sev-info",
  mssql:    "bg-sev-info/15 text-sev-info",
  redis:    "bg-sev-critical/15 text-sev-critical",
  telnet:   "bg-sev-critical/15 text-sev-critical",
};
                ...
                <div key={rowKey} className="rounded-lg border border-border bg-card p-3 text-sm">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="font-mono text-xs font-semibold tabular-nums text-foreground">{p.port}</span>
                      <span className="font-mono text-[10px] uppercase tracking-wide text-muted-foreground">{p.protocol}</span>
                    </div>
                  </div>
                  {showAsset && (
                  ...
                  {p.ip && <p className="mt-1 font-mono text-[10px] tabular-nums text-muted-foreground">{p.ip}</p>}
                </div>
              );
            })}
          </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border bg-muted/30 text-left font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                <th className="cursor-pointer px-3 py-2 hover:text-foreground select-none" onClick={() => handleSort("host")}>
                ...
                <th className="px-3 py-2">Subdomains</th>
                <th className="cursor-pointer px-3 py-2 hover:text-foreground select-none" onClick={() => handleSort("port")}>
                  <span className="inline-flex items-center gap-1">Port <SortIcon dir={hook.sortBy === "port" ? hook.sortDir : null} /></span>
                </th>
                <th className="px-3 py-2">Protocol</th>
                <th className="cursor-pointer px-3 py-2 hover:text-foreground select-none" onClick={() => handleSort("service")}>
                  <span className="inline-flex items-center gap-1">Service <SortIcon dir={hook.sortBy === "service" ? hook.sortDir : null} /></span>
                </th>
                {!showAsset && <th className="px-3 py-2">Standard</th>}
              </tr>
            </thead>
            <tbody>
              {hook.data.map((p) => {
                ...
                const overflow = p.subdomains.length - SUBDOMAIN_PREVIEW;
                return (
                  <tr key={rowKey} className="border-b border-border/50 hover:bg-surface-hover transition-colors">
                    ...
                    <td className="px-3 py-2 font-mono tabular-nums text-foreground">{p.port}</td>
                    <td className="px-3 py-2 font-mono uppercase tracking-wide text-muted-foreground">{p.protocol}</td>
                    <td className="px-3 py-2">
                      {p.service ? (
                    ...
                    {!showAsset && (
                      <td className="px-3 py-2">
                        {p.standard ? (
                          <span className="rounded px-1.5 py-0.5 font-mono text-[10px] font-medium uppercase tracking-wide bg-muted text-muted-foreground">standard</span>
                        ) : (
                          <span className="rounded px-1.5 py-0.5 font-mono text-[10px] font-medium uppercase tracking-wide bg-sev-medium/15 text-sev-medium">non-standard</span>
                        )}
                      </td>
                    )}
                  </tr>
